...
export class RunMinecraft {
    ...
    async runInstalledVersion (name: string, account: Account): Promise<void> {
        ...
        const versionObject: McInstallation = expandInheritsFrom(JSON.parse(fs.readFileSync(`${this.launcher.rootPath}/versions/${name}/${name}.json`).toString()), this.launcher.rootPath);
