    ...
    private async installAssets (asset: AssetIndexInfo): Promise<void> {
        ...
        for (const assid in assetobj.objects) {
            ...
            if (!fs.existsSync(`${this.launcher.rootPath}/assets/objects/${assitem.hash.slice(0, 2)}/${assitem.hash}`) ||
                ...
                console.log(assid);
                await downloads.download(`https://resources.download.minecraft.net/${assitem.hash.slice(0, 2)}/${assitem.hash}`, `${this.launcher.rootPath}/assets/objects/${assitem.hash.slice(0, 2)}/${assitem.hash}`);
            }
        }
    }
